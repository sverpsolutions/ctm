...
export async function getDashboardData(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // 1. KPI Counts
    const kpiResult = await executeQuery(
      `SELECT
    ...
    const dateKpiResult = await executeQuery(
      `SELECT
        COUNT(*) AS TotalDates,
    ...
    const upcomingDatesResult = await executeQuery(
      `SELECT TOP 10 
    ...
    const employeeCompResult = await executeQuery(
      `SELECT TOP 6
        e.EmployeeName,
    ...
    let myDayTasks: any[] = [];
    if (employeeId) {
      const myDayResult = await executeQuery(
        `SELECT TOP 10
          t.TaskID, t.TaskNumber, t.TaskTitle, t.DueDate, t.Priority, t.Status, t.PercentageComplete,
