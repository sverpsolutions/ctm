...
export async function getPlatformDashboard(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const tenants = await executeQuery<any>(
      `SELECT
    ...
    const companies = await executeQuery<any>(
      `SELECT COUNT(*) AS TotalCompanies FROM dbo.Companies WHERE IsDeleted = 0`
    );

    const users = await executeQuery<any>(
      `SELECT COUNT(*) AS TotalUsers FROM dbo.Users WHERE IsDeleted = 0 AND IsPlatformAdmin = 0`
    );
    ...
    const recentTenants = await executeQuery<any>(
      `SELECT TOP 5 TenantID, TenantCode, TenantName, Status, CreatedAt
       FROM dbo.Tenants WHERE IsDeleted = 0
       ORDER BY CreatedAt DESC`
    );

    const recentRegistrations = await executeQuery<any>(
      `SELECT TOP 5 RegistrationID, CompanyName, ContactPerson, ContactEmail, Status, CreatedAt
       FROM dbo.TenantRegistrations
       ORDER BY CreatedAt DESC`
    );
...
export async function getTenants(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const countResult = await executeQuery<any>(
      `SELECT COUNT(*) AS Total FROM dbo.Tenants t ${whereClause}`,
      params
    );

    const tenantsResult = await executeQuery<any>(
      `SELECT t.*,
...
export async function getTenantById(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const { id } = req.params;
    const tenantResult = await executeQuery<any>(
      `SELECT t.*,
        (SELECT COUNT(*) FROM dbo.Companies c WHERE c.TenantID = t.TenantID AND c.IsDeleted = 0) AS CompanyCount,
        (SELECT COUNT(*) FROM dbo.Users u WHERE u.TenantID = t.TenantID AND u.IsDeleted = 0 AND u.IsPlatformAdmin = 0) AS UserCount
       FROM dbo.Tenants t
    ...
    const companiesResult = await executeQuery<any>(
      `SELECT CompanyID, CompanyCode, CompanyName, CompanyType, Status, City, State
       FROM dbo.Companies WHERE TenantID = @tenantId AND IsDeleted = 0 ORDER BY CompanyName`,
      { tenantId: parseInt(id, 10) }
    );
...
export async function createTenant(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const {
      tenantCode, tenantName, legalName, contactPerson, contactEmail, contactMobile,
      industry, address, city, state, country, pinCode, website, gstin, pan,
      subscriptionTier, maxCompanies, maxUsers, licenseStartDate, licenseEndDate,
      enabledModules, status,
    } = req.body;
    ...
    const existing = await executeQuery<any>(
      `SELECT TenantID FROM dbo.Tenants WHERE TenantCode = @code AND IsDeleted = 0`,
    ...
    const result = await executeQuery<any>(
      `INSERT INTO dbo.Tenants (TenantCode, TenantName, LegalName, ContactPerson, ContactEmail, ContactMobile, Industry, Address, City, State, Country, PINCode, Website, GSTIN, PAN, SubscriptionTier, MaxCompanies, MaxUsers, LicenseStartDate, LicenseEndDate, EnabledModules, Status, CreatedBy)
...
export async function updateTenant(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const {
      tenantName, legalName, contactPerson, contactEmail, contactMobile,
      industry, address, city, state, country, pinCode, website, gstin, pan,
      subscriptionTier, maxCompanies, maxUsers, licenseStartDate, licenseEndDate,
      enabledModules, status,
    } = req.body;
    ...
    const existing = await executeQuery<any>(
      `SELECT * FROM dbo.Tenants WHERE TenantID = @tenantId AND IsDeleted = 0`,
      { tenantId }
    );
    if (existing.recordset.length === 0) {
      res.status(404).json({ success: false, message: 'Tenant not found.' });
...
export async function suspendTenant(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    await executeQuery(
      `UPDATE dbo.Tenants SET Status = 'Suspended', UpdatedBy = @userId, UpdatedAt = SYSUTCDATETIME() WHERE TenantID = @tenantId AND IsDeleted = 0`,
      { tenantId, userId: req.user!.userId }
    );
    await logAudit({ userId: req.user!.userId, username: req.user!.username, action: 'SUSPEND_TENANT', entityName: 'Tenants', entityId: tenantId, req });
...
export async function activateTenant(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const tenantId = parseInt(req.params.id, 10);
    await executeQuery(
      `UPDATE dbo.Tenants SET Status = 'Active', UpdatedBy = @userId, UpdatedAt = SYSUTCDATETIME() WHERE TenantID = @tenantId AND IsDeleted = 0`,
      { tenantId, userId: req.user!.userId }
    );
    await logAudit({ userId: req.user!.userId, username: req.user!.username, action: 'ACTIVATE_TENANT', entityName: 'Tenants', entityId: tenantId, req });
...
export async function resetTenantAdminPassword(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const tenantId = parseInt(req.params.id, 10);
    ...
    const adminUser = await executeQuery<any>(
      `SELECT TOP 1 u.UserID, u.Username FROM dbo.Users u
    ...
    await executeQuery(
      `UPDATE dbo.Users SET PasswordHash = @hash, MustChangePassword = 1, UpdatedAt = SYSUTCDATETIME() WHERE UserID = @userId`,
      { hash, userId: adminUser.recordset[0].UserID }
    );
...
export async function getAllUsers(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const countResult = await executeQuery<any>(
      `SELECT COUNT(*) AS Total FROM dbo.Users u LEFT JOIN dbo.Employees e ON u.EmployeeID = e.EmployeeID ${whereClause}`,
      params
    );

    const usersResult = await executeQuery<any>(
      `SELECT u.UserID, u.Username, u.Email, u.Status, u.LastLoginAt, u.TenantID,
...
export async function createRegistrationLink(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    await executeQuery(
      `INSERT INTO dbo.TenantRegistrations (RegistrationToken, TokenExpiresAt, CompanyName, ContactPerson, ContactEmail, Notes, Status)
       VALUES (@token, @expiresAt, '', '', '', @notes, 'LinkGenerated')`,
      { token, expiresAt, notes: notes || null }
    );
...
export async function getRegistrations(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const countResult = await executeQuery<any>(
      `SELECT COUNT(*) AS Total FROM dbo.TenantRegistrations r ${whereClause}`,
      params
    );

    const regsResult = await executeQuery<any>(
      `SELECT r.*, t.TenantName
...
export async function getRegistrationById(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const { id } = req.params;
    const result = await executeQuery<any>(
      `SELECT r.*, t.TenantName FROM dbo.TenantRegistrations r LEFT JOIN dbo.Tenants t ON r.TenantID = t.TenantID WHERE r.RegistrationID = @regId`,
      { regId: parseInt(id, 10) }
    );
...
export async function approveRegistration(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const regResult = await executeQuery<any>(
      `SELECT * FROM dbo.TenantRegistrations WHERE RegistrationID = @regId`,
      { regId }
    );
    ...
    const tenantResult = await executeQuery<any>(
      `INSERT INTO dbo.Tenants (TenantCode, TenantName, LegalName, ContactPerson, ContactEmail, ContactMobile, Industry, Address, City, State, Country, PINCode, Website, GSTIN, PAN, SubscriptionTier, MaxCompanies, MaxUsers, EnabledModules, Status, CreatedBy)
    ...
    const companyResult = await executeQuery<any>(
      `INSERT INTO dbo.Companies (TenantID, CompanyCode, CompanyName, LegalName, CompanyType, ShortName, City, State, Country, Status, EnabledModules, SubscriptionTier, MaxUsers)
    ...
    const saRole = await executeQuery<any>(
      `SELECT RoleID FROM dbo.Roles WHERE RoleName = 'Super Admin' AND (IsPlatformRole = 0 OR IsPlatformRole IS NULL) LIMIT 1`
    );
    ...
    const passwordHash = reg.PasswordHash || bcrypt.hashSync('ChangeMe@123', bcrypt.genSaltSync(12));

    const userResult = await executeQuery<any>(
      `INSERT INTO dbo.Users (CompanyID, Username, Email, PasswordHash, RoleID, Status, TenantID, IsPlatformAdmin, MustChangePassword)
    ...
    await executeQuery(
      `INSERT INTO dbo.UserCompany (UserID, CompanyID, RoleID, AccessScope, IsPrimary, IsActive)
       VALUES (@userId, @companyId, @roleId, 'Global', 1, 1)`,
      { userId: newUserId, companyId: newCompanyId, roleId: saRoleId }
    );
    ...
    for (const dept of ['Administration', 'Finance', 'Operations', 'Human Resources', 'IT']) {
      ...
      await executeQuery(
        `INSERT INTO dbo.Departments (CompanyID, DepartmentCode, DepartmentName, Status)
         VALUES (@companyId, @code, @name, 'Active')`,
        { companyId: newCompanyId, code: deptCode, name: dept }
      );
    }

    // 7. Update registration
    await executeQuery(
      `UPDATE dbo.TenantRegistrations SET TenantID = @tenantId, Status = 'Approved', ReviewedBy = @reviewedBy, ReviewedAt = SYSUTCDATETIME(), UpdatedAt = SYSUTCDATETIME()
       WHERE RegistrationID = @regId`,
      { tenantId: newTenantId, reviewedBy: req.user!.userId, regId }
    );
...
export async function rejectRegistration(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    await executeQuery(
      `UPDATE dbo.TenantRegistrations SET Status = 'Rejected', RejectionReason = @reason, ReviewedBy = @reviewedBy, ReviewedAt = SYSUTCDATETIME(), UpdatedAt = SYSUTCDATETIME()
       WHERE RegistrationID = @regId`,
      { reason: reason || 'No reason provided', reviewedBy: req.user!.userId, regId }
    );
