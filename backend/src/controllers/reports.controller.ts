...
export async function getTaskReports(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
...
export async function getDateReports(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const scopeIds = req.tenant?.scopeCompanyIds || [req.user!.companyId];
    ...
    const renewalHistory = await executeQuery(
      `SELECT 
...
export async function getDepartmentPerformance(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const scopeIds = req.tenant?.scopeCompanyIds || [req.user!.companyId];
