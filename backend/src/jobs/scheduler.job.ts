...
export async function runDailyMaintenanceJob(): Promise<void> {
  ...
  try {
    ...
    const overdueResult = await executeQuery<{
      ...
      `SELECT t.TaskID, t.TaskNumber, t.TaskTitle, t.DueDate, t.ManagerID, t.CompanyID
       FROM dbo.Tasks t
    ...
    for (const task of overdueResult.recordset) {
      ...
      await executeQuery(
        `UPDATE dbo.Tasks SET Status = N'Overdue', UpdatedAt = SYSUTCDATETIME() WHERE TaskID = @id`,
        { id: task.TaskID }
      );
      ...
      await executeQuery(
        `INSERT INTO dbo.TaskActivityLog (TaskID, UserID, ActionType, Description)
         VALUES (@id, 1, N'OverdueAutoFlag', N'Task automatically marked as Overdue by system scheduler.')`,
        { id: task.TaskID }
      );
    }

    // 2. OVERDUE TASK ESCALATIONS
    const escalatedTasks = await executeQuery<{
      TaskID: number;
      TaskNumber: string;
      TaskTitle: string;
      DueDate: Date;
    ...
    for (const t of escalatedTasks.recordset) {
      ...
      const assignees = await executeQuery<{ UserID: number }>(
        `SELECT u.UserID FROM dbo.TaskAssignees ta JOIN dbo.Users u ON ta.EmployeeID = u.EmployeeID WHERE ta.TaskID = @id AND u.Status = 'Active'`,
        { id: t.TaskID }
      );
      ...
      if (t.DaysOverdue >= 2 && t.ManagerID) {
        const mgrUser = await executeQuery<{ UserID: number }>(
          `SELECT UserID FROM dbo.Users WHERE EmployeeID = @mgrId AND Status = 'Active'`,
          { mgrId: t.ManagerID }
        );
    ...
    const approachingDates = await executeQuery<{
    ...
    for (const d of approachingDates.recordset) {
      if (d.ResponsibleEmployeeID) {
        const respUser = await executeQuery<{ UserID: number }>(
          `SELECT UserID FROM dbo.Users WHERE EmployeeID = @empId AND Status = 'Active'`,
          { empId: d.ResponsibleEmployeeID }
        );
    ...
    for (const at of autoTasks.recordset) {
      ...
      const countRes = await executeQuery<{ Cnt: number }>(
        `SELECT COUNT(*) AS Cnt FROM dbo.Tasks WHERE TaskNumber LIKE @pattern`,
      ...
      // Create Task
      const insertTask = await executeQuery<{ TaskID: number }>(
      ...
      if (at.TaskAssignedToID) {
        await executeQuery(
          `INSERT INTO dbo.TaskAssignees (TaskID, EmployeeID) VALUES (@genTaskId, @empId)`,
          { genTaskId, empId: at.TaskAssignedToID }
        );
      }

      // Link generated task back to Important Date
      await executeQuery(
        `UPDATE dbo.ImportantDates SET GeneratedTaskID = @genTaskId WHERE ImportantDateID = @dateId`,
        { genTaskId, dateId: at.ImportantDateID }
      );
