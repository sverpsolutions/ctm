...
exports.getImportantDateById = getImportantDateById;
exports.createImportantDate = createImportantDate;
exports.updateImportantDate = updateImportantDate;
exports.renewImportantDate = renewImportantDate;
...
async function getImportantDates(req, res, next) {
    try {
        ...
        const { page = '1', limit = '25', companyId: filterCompanyId, search, categoryId, departmentId, locationId, status, timeframe, // critical (0-3d), urgent (4-7d), upcoming (8-30d), future (31+d), expired, all
        sortBy = 'ExpiryDate', sortOrder = 'ASC', } = req.query;
        const pageNum = Math.max(1, parseInt(page, 10) || 1);
        const limitNum = Math.min(100, Math.max(1, parseInt(limit, 10) || 25));
        const offset = (pageNum - 1) * limitNum;
        let whereClauses = ['d.IsDeleted = 0'];
       ...
       WHERE ${whereSql}`, params);
        ...
        const dataResult = await (0, db_1.executeQuery)(`SELECT 
...
async function getImportantDateById(req, res, next) {
    try {
        const id = parseInt(req.params.id, 10);
        const dateResult = await (0, db_1.executeQuery)(`SELECT 
        d.*,
        comp.CompanyName, comp.CompanyCode,
        c.CategoryName, c.ColorCode AS CategoryColor, c.IconName AS CategoryIcon,
        e.EmployeeName AS ResponsiblePersonName, e.Email AS ResponsiblePersonEmail,
       ...
       WHERE d.ImportantDateID = @id AND d.IsDeleted = 0`, { id });
        ...
        const remindersResult = await (0, db_1.executeQuery)(`SELECT * FROM dbo.ImportantDateReminders WHERE ImportantDateID = @id ORDER BY DaysBefore DESC`, { id });
        // Renewal History
        const historyResult = await (0, db_1.executeQuery)(`SELECT h.*, u.Username AS RenewedByUsername
       FROM dbo.ImportantDateHistory h
       LEFT JOIN dbo.Users u ON h.RenewedByUserID = u.UserID
       WHERE h.ImportantDateID = @id
       ORDER BY h.RenewedDate DESC`, { id });
        // Related Tasks
        const tasksResult = await (0, db_1.executeQuery)(`SELECT TaskID, TaskNumber, TaskTitle, Status, Priority, DueDate, PercentageComplete
       FROM dbo.Tasks
...
async function createImportantDate(req, res, next) {
    try {
        const userId = req.user.userId;
        const { companyId, categoryId, title, description, locationId, departmentId, relatedEmployeeId, relatedVendor, relatedCustomer, referenceNumber, date, startDate, expiryDate, recurrenceType = 'Does Not Repeat', responsibleEmployeeId, priority = 'Medium', notes, attachment, autoGenerateTask = false, leadDaysForTask = 15, taskAssignedToId, reminders = [30, 15, 7, 1], // default reminder days
         } = req.body;
        ...
        const insertResult = await (0, db_1.executeQuery)(`INSERT INTO dbo.ImportantDates (
      ...
      VALUES (
        ...
        if (Array.isArray(reminders)) {
            for (const days of reminders) {
                await (0, db_1.executeQuery)(`INSERT INTO dbo.ImportantDateReminders (ImportantDateID, DaysBefore, ReminderChannel)
           VALUES (@newDateId, @days, N'In-App')`, { newDateId, days: parseInt(days, 10) });
            }
        }
...
async function updateImportantDate(req, res, next) {
    try {
        const id = parseInt(req.params.id, 10);
        const userId = req.user.userId;
        const { companyId, categoryId, title, description, locationId, departmentId, relatedEmployeeId, relatedVendor, relatedCustomer, referenceNumber, date, startDate, expiryDate, recurrenceType, responsibleEmployeeId, priority, notes, attachment, status, autoGenerateTask, leadDaysForTask, taskAssignedToId, } = req.body;
        const existing = await (0, db_1.executeQuery)(`SELECT * FROM dbo.ImportantDates WHERE ImportantDateID = @id AND IsDeleted = 0`, { id });
        ...
        await (0, db_1.executeQuery)(`UPDATE dbo.ImportantDates SET
...
async function renewImportantDate(req, res, next) {
    try {
        const id = parseInt(req.params.id, 10);
        const userId = req.user.userId;
        ...
        const dateResult = await (0, db_1.executeQuery)(`SELECT * FROM dbo.ImportantDates WHERE ImportantDateID = @id AND IsDeleted = 0`, { id });
        if (dateResult.recordset.length === 0) {
            res.status(404).json({ success: false, message: 'Important Date record not found.' });
            return;
        }
        ...
        await (0, db_1.executeQuery)(`INSERT INTO dbo.ImportantDateHistory (
        ...
        UpdatedBy = @userId,
        UpdatedAt = SYSUTCDATETIME()
       WHERE ImportantDateID = @id`, { id, newExpiryDate, userId });
        await (0, audit_service_1.logAudit)({
            userId,
...
async function deleteImportantDate(req, res, next) {
    try {
        const id = parseInt(req.params.id, 10);
        const userId = req.user.userId;
        const existing = await (0, db_1.executeQuery)(`SELECT ImportantDateID, CompanyID FROM dbo.ImportantDates WHERE ImportantDateID = @id AND IsDeleted = 0`, { id });
        if (existing.recordset.length === 0) {
            res.status(404).json({ success: false, message: 'Important Date not found.' });
            return;
        }
        ...
        await (0, db_1.executeQuery)(`UPDATE dbo.ImportantDates SET IsDeleted = 1, UpdatedBy = @userId, UpdatedAt = SYSUTCDATETIME() 
       WHERE ImportantDateID = @id`, { id, userId });
        await (0, audit_service_1.logAudit)({
            userId,
...
async function getCategories(req, res, next) {
    try {
        ...
        const categories = await (0, db_1.executeQuery)(`SELECT * FROM dbo.ImportantDateCategories 
       WHERE (CompanyID = @companyId OR CompanyID IS NULL) AND Status = 'Active'
       ORDER BY IsSystemDefault DESC, CategoryName ASC`, { companyId });
...
async function createCategory(req, res, next) {
    try {
        ...
        const insertResult = await (0, db_1.executeQuery)(`INSERT INTO dbo.ImportantDateCategories (CompanyID, CategoryName, ColorCode, IconName, IsSystemDefault, Status, CreatedAt)
       OUTPUT INSERTED.CategoryID
       VALUES (@companyId, @categoryName, @colorCode, @iconName, 0, N'Active', SYSUTCDATETIME())`, { companyId, categoryName: categoryName.trim(), colorCode, iconName });
        res.status(201).json({
            success: true,
