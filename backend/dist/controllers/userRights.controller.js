...
exports.getUsersForCompanyRights = getUsersForCompanyRights;
exports.getUserCompanyRights = getUserCompanyRights;
exports.saveUserCompanyRights = saveUserCompanyRights;
const db_1 = require("../config/db");
const audit_service_1 = require("../services/audit.service");
/**
 * Get users list with their current company access summary for User Company Rights screen
 */
async function getUsersForCompanyRights(req, res, next) {
    try {
 ...
 */
async function getUserCompanyRights(req, res, next) {
    try {
        ...
        // Fetch user details
        const userResult = await (0, db_1.executeQuery)(`SELECT u.UserID, u.Username, u.Email, u.RoleID, r.RoleName, u.CompanyID,
              e.EmployeeName, e.Designation
       FROM dbo.Users u
       LEFT JOIN dbo.Roles r ON u.RoleID = r.RoleID
       LEFT JOIN dbo.Employees e ON u.EmployeeID = e.EmployeeID
       WHERE u.UserID = @targetUserId AND u.IsDeleted = 0`, { targetUserId });
        ...
        const companiesResult = await (0, db_1.executeQuery)(`SELECT c.CompanyID, c.CompanyCode, c.CompanyName, c.LegalName, c.CompanyType, c.ParentCompanyID,
       ...
       ORDER BY c.CompanyName ASC`);
        ...
        const companies = companiesResult.recordset.map((comp) => ({
 ...
 */
async function saveUserCompanyRights(req, res, next) {
    try {
        const targetUserId = parseInt(req.params.userId, 10);
        ...
        const userResult = await (0, db_1.executeQuery)(`SELECT UserID, Username, RoleID, CompanyID, EmployeeID FROM dbo.Users WHERE UserID = @targetUserId AND IsDeleted = 0`, { targetUserId });
        if (userResult.recordset.length === 0) {
            res.status(404).json({ success: false, message: 'User not found.' });
            return;
        }
        ...
        if (!isSuperAdmin) {
            const unauthorized = companyIds.filter((cid) => !allowedCompanyIds.includes(cid));
        ...
        let newPrimaryId = primaryCompanyId ? parseInt(primaryCompanyId, 10) : null;
        ...
        if (companyIds.length === 0) {
            await (0, db_1.executeQuery)(`UPDATE dbo.tbl_user_companies SET is_active = 0, updated_by = @adminUserId, updated_at = SYSUTCDATETIME()
         WHERE user_id = @targetUserId ${scopeWhere}`, { targetUserId, adminUserId });
            await (0, db_1.executeQuery)(`UPDATE dbo.UserCompany SET IsActive = 0, UpdatedAt = SYSUTCDATETIME()
         WHERE UserID = @targetUserId ${isSuperAdmin ? '' : `AND CompanyID IN (${allowedCompanyIds.join(', ')})`}`, { targetUserId });
        }
        else {
            const placeholders = companyIds.map((_, idx) => `@cid_${idx}`).join(', ');
            const cidParams = { targetUserId, adminUserId };
            companyIds.forEach((cid, idx) => {
                cidParams[`cid_${idx}`] = cid;
            });
            await (0, db_1.executeQuery)(`UPDATE dbo.tbl_user_companies SET is_active = 0, updated_by = @adminUserId, updated_at = SYSUTCDATETIME()
         WHERE user_id = @targetUserId AND company_id NOT IN (${placeholders}) ${scopeWhere}`, cidParams);
            await (0, db_1.executeQuery)(`UPDATE dbo.UserCompany SET IsActive = 0, UpdatedAt = SYSUTCDATETIME()
         WHERE UserID = @targetUserId AND CompanyID NOT IN (${placeholders}) ${isSuperAdmin ? '' : `AND CompanyID IN (${allowedCompanyIds.join(', ')})`}`, cidParams);
        }
        ...
        for (const compId of companyIds) {
            ...
            const checkTbl = await (0, db_1.executeQuery)(`SELECT id FROM dbo.tbl_user_companies WHERE user_id = @targetUserId AND company_id = @compId`, { targetUserId, compId });
            if (checkTbl.recordset.length > 0) {
                await (0, db_1.executeQuery)(`UPDATE dbo.tbl_user_companies 
           SET is_active = 1, updated_by = @adminUserId, updated_at = SYSUTCDATETIME()
           WHERE user_id = @targetUserId AND company_id = @compId`, { targetUserId, compId, adminUserId });
            }
            else {
                await (0, db_1.executeQuery)(`INSERT INTO dbo.tbl_user_companies (user_id, company_id, is_active, created_by, created_at, updated_by, updated_at)
           VALUES (@targetUserId, @compId, 1, @adminUserId, SYSUTCDATETIME(), @adminUserId, SYSUTCDATETIME())`, { targetUserId, compId, adminUserId });
            }
            // Upsert UserCompany for backwards compatibility
            const checkUc = await (0, db_1.executeQuery)(`SELECT UserCompanyID FROM dbo.UserCompany WHERE UserID = @targetUserId AND CompanyID = @compId`, { targetUserId, compId });
            if (checkUc.recordset.length > 0) {
                await (0, db_1.executeQuery)(`UPDATE dbo.UserCompany 
        ...
        if (newPrimaryId) {
            await (0, db_1.executeQuery)(`UPDATE dbo.Users SET CompanyID = @newPrimaryId, UpdatedAt = SYSUTCDATETIME() WHERE UserID = @targetUserId`, { newPrimaryId, targetUserId });
            if (targetUser.EmployeeID) {
                await (0, db_1.executeQuery)(`UPDATE dbo.Employees SET CompanyID = @newPrimaryId, UpdatedAt = SYSUTCDATETIME() WHERE EmployeeID = @employeeId`, { newPrimaryId, employeeId: targetUser.EmployeeID });
            }
            // Ensure only the designated company is marked IsPrimary in UserCompany
            await (0, db_1.executeQuery)(`UPDATE dbo.UserCompany SET IsPrimary = CASE WHEN CompanyID = @newPrimaryId THEN 1 ELSE 0 END WHERE UserID = @targetUserId`, { newPrimaryId, targetUserId });
        }
        // 4. Audit Log
