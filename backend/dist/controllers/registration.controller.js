...
async function validateRegistrationToken(req, res, next) {
    try {
        const { token } = req.params;
        const result = await (0, db_1.executeQuery)(`SELECT RegistrationID, Status, TokenExpiresAt, CompanyName, ContactPerson, ContactEmail
...
async function submitRegistration(req, res, next) {
    try {
        const { token } = req.params;
        const { companyName, legalName, contactPerson, contactEmail, contactMobile, address, city, state, country, pinCode, gstin, pan, website, industry, companyType, numBranches, numUsers, requestedModules, adminName, adminEmail, adminUsername, password, } = req.body;
        ...
        const result = await (0, db_1.executeQuery)(`SELECT RegistrationID, Status, TokenExpiresAt FROM dbo.TenantRegistrations WHERE RegistrationToken = @token`, { token });
        if (result.recordset.length === 0) {
            res.status(404).json({ success: false, message: 'Invalid registration link.' });
            return;
        }
        ...
        const existingUser = await (0, db_1.executeQuery)(`SELECT UserID FROM dbo.Users WHERE (Username = @username OR Email = @email) AND IsDeleted = 0`, { username: adminUsername || adminEmail || contactEmail, email: adminEmail || contactEmail });
        ...
        await (0, db_1.executeQuery)(`UPDATE dbo.TenantRegistrations SET
