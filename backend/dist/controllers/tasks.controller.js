...
exports.bulkTaskAction = bulkTaskAction;
...
async function getTasks(req, res, next) {
    try {
        const { page = '1', limit = '25', search, status, priority, departmentId, locationId, categoryId, employeeId, myTasks, dueDateFrom, dueDateTo, companyId, sortBy = 'DueDate', sortOrder = 'ASC', } = req.query;
        ...
        let whereClauses = ['t.IsDeleted = 0'];
        const params = { limitNum, offset };
        // Strict Company Filter Enforcement
        if (companyId) {
            const filterCompId = parseInt(companyId, 10);
        ...
        if (locationId) {
            ...
            params.locationId = parseInt(locationId, 10);
        }
        if (categoryId) {
        ...
        if (employeeId || myTasks === 'true') {
            const targetEmpId = myTasks === 'true' ? req.user.employeeId : parseInt(employeeId, 10);
            if (targetEmpId) {
        ...
        const sortCol = allowedSortCols[sortBy] || 't.DueDate';
        const orderDir = sortOrder.toUpperCase() === 'DESC' ? 'DESC' : 'ASC';
        // Total Count Query
        const countResult = await (0, db_1.executeQuery)(`SELECT COUNT(*) AS Total 
       FROM dbo.Tasks t
    ...
    `;
        const dataResult = await (0, db_1.executeQuery)(query, params);
        res.json({
            success: true,
...
async function getTaskById(req, res, next) {
    try {
        ...
        const taskResult = await (0, db_1.executeQuery)(`SELECT 
        t.*,
        CASE 
          WHEN (t.DueDate < CAST(GETDATE() AS DATE) AND t.Status NOT IN ('Completed', 'Cancelled')) THEN 'Overdue'
          ELSE t.Status 
       ...
       WHERE t.TaskID = @taskId AND t.IsDeleted = 0`, { taskId });
        ...
        // Fetch Assignees
        const assigneesResult = await (0, db_1.executeQuery)(`SELECT e.EmployeeID, e.EmployeeName, e.EmployeeCode, e.Email, e.Designation, e.ProfilePhoto
       FROM dbo.TaskAssignees ta
       JOIN dbo.Employees e ON ta.EmployeeID = e.EmployeeID
       WHERE ta.TaskID = @taskId`, { taskId });
        // Fetch Checklist Items
        const checklistResult = await (0, db_1.executeQuery)(`SELECT tc.ChecklistItemID, tc.Title, tc.IsCompleted, tc.CompletedAt, tc.SortOrder,
       ...
       ORDER BY tc.SortOrder ASC, tc.ChecklistItemID ASC`, { taskId });
        // Fetch Progress Updates History
        const updatesResult = await (0, db_1.executeQuery)(`SELECT tu.UpdateID, tu.PreviousStatus, tu.NewStatus, tu.PreviousProgress, tu.NewProgress,
       ...
       ORDER BY tu.CreatedAt DESC`, { taskId });
        // Fetch Comments
        const commentsResult = await (0, db_1.executeQuery)(`SELECT tc.CommentID, tc.CommentText, tc.CreatedAt,
              u.Username, u.UserID,
       ...
       WHERE tc.TaskID = @taskId
       ORDER BY tc.CreatedAt ASC`, { taskId });
        // Fetch Attachments
        const attachmentsResult = await (0, db_1.executeQuery)(`SELECT att.AttachmentID, att.FileName, att.OriginalName, att.FileType, att.FileSize,
       ...
       ORDER BY att.CreatedAt DESC`, { taskId });
        // Fetch Activity Log
        const activityResult = await (0, db_1.executeQuery)(`SELECT act.ActivityID, act.ActionType, act.FieldName, act.OldValue, act.NewValue,
              act.Description, act.CreatedAt,
              u.Username
       FROM dbo.TaskActivityLog act
       JOIN dbo.Users u ON act.UserID = u.UserID
       WHERE act.TaskID = @taskId
       ORDER BY act.CreatedAt DESC`, { taskId });
        res.json({
            success: true,
            data: {
                ...task,
...
async function createTask(req, res, next) {
    try {
        ...
        const { companyId, title, taskTitle, description, taskDescription, taskType = 'General', reminderDate, remarks, status = 'New', departmentId, locationId, categoryId, managerId, startDate, dueDate, priority = 'Medium', estimatedHours = 0, assigneeIds = [], checklistItems = [], tags, relatedImportantDateId, relatedVendor, relatedCustomer, } = req.body;
        ...
        const countResult = await (0, db_1.executeQuery)(`SELECT COUNT(*) AS TaskCount FROM dbo.Tasks WHERE TaskNumber LIKE @pattern`, { pattern: `TSK-${currentYear}-%` });
        const nextNum = (countResult.recordset[0]?.TaskCount || 0) + 1;
        const taskNumber = `TSK-${currentYear}-${String(nextNum).padStart(4, '0')}`;
        const insertResult = await (0, db_1.executeQuery)(`INSERT INTO dbo.Tasks (
      ...
      VALUES (
        ...
        if (Array.isArray(assigneeIds) && assigneeIds.length > 0) {
            for (const empId of assigneeIds) {
                await (0, db_1.executeQuery)(`INSERT INTO dbo.TaskAssignees (TaskID, EmployeeID) VALUES (@newTaskId, @empId)`, { newTaskId, empId: parseInt(empId, 10) });
                // Notify Assignee
                const empUser = await (0, db_1.executeQuery)(`SELECT UserID FROM dbo.Users WHERE EmployeeID = @empId AND Status = 'Active'`, { empId });
        ...
        if (Array.isArray(checklistItems) && checklistItems.length > 0) {
            ...
            for (const itemTitle of checklistItems) {
                if (typeof itemTitle === 'string' && itemTitle.trim()) {
                    await (0, db_1.executeQuery)(`INSERT INTO dbo.TaskChecklist (TaskID, Title, IsCompleted, SortOrder)
             VALUES (@newTaskId, @itemTitle, 0, @sortOrder)`, { newTaskId, itemTitle: itemTitle.trim(), sortOrder });
                    sortOrder++;
                }
            }
        }
       ...
       VALUES (@newTaskId, @userId, N'Created', N'Task created and initial assignees notified.')`, { newTaskId, userId });
...
async function updateTask(req, res, next) {
    try {
        const taskId = parseInt(req.params.id, 10);
        const userId = req.user.userId;
        const { companyId, title, description, taskType, reminderDate, remarks, departmentId, locationId, categoryId, managerId, startDate, dueDate, priority, status, percentageComplete, estimatedHours, actualHours, assigneeIds, tags, relatedVendor, relatedCustomer, } = req.body;
        const existingTask = await (0, db_1.executeQuery)(`SELECT * FROM dbo.Tasks WHERE TaskID = @taskId AND IsDeleted = 0`, { taskId });
        ...
        await (0, db_1.executeQuery)(`UPDATE dbo.Tasks SET
       ...
       WHERE TaskID = @taskId`, {
        ...
        if (Array.isArray(assigneeIds)) {
            await (0, db_1.executeQuery)(`DELETE FROM dbo.TaskAssignees WHERE TaskID = @taskId`, { taskId });
            for (const empId of assigneeIds) {
                await (0, db_1.executeQuery)(`INSERT INTO dbo.TaskAssignees (TaskID, EmployeeID) VALUES (@taskId, @empId)`, { taskId, empId: parseInt(empId, 10) });
            }
        }
        // Log Activity Changes
        if (status && status !== old.Status) {
            await (0, db_1.executeQuery)(`INSERT INTO dbo.TaskActivityLog (TaskID, UserID, ActionType, FieldName, OldValue, NewValue, Description)
...
async function updateProgress(req, res, next) {
    try {
        ...
        const taskResult = await (0, db_1.executeQuery)(`SELECT TaskID, CompanyID, Status, PercentageComplete, ActualHours, TaskNumber, TaskTitle, ManagerID 
       FROM dbo.Tasks WHERE TaskID = @taskId AND IsDeleted = 0`, { taskId });
        if (taskResult.recordset.length === 0) {
            res.status(404).json({ success: false, message: 'Task not found.' });
            return;
        }
        ...
        await (0, db_1.executeQuery)(`INSERT INTO dbo.TaskUpdates (
        ...
        UpdatedAt = SYSUTCDATETIME()
       WHERE TaskID = @taskId`, { newStatus, newProgress, newActualHours, taskId });
        ...
        if (Array.isArray(checklistUpdates) && checklistUpdates.length > 0) {
            for (const item of checklistUpdates) {
                await (0, db_1.executeQuery)(`UPDATE dbo.TaskChecklist 
       ...
       VALUES (@taskId, @userId, N'ProgressUpdate', @desc)`, {
        ...
        if (newStatus === 'Waiting for Approval' && task.ManagerID) {
            const mgrUser = await (0, db_1.executeQuery)(`SELECT UserID FROM dbo.Users WHERE EmployeeID = @managerId AND Status = 'Active'`, { managerId: task.ManagerID });
...
async function addComment(req, res, next) {
    try {
        ...
        const taskCheck = await (0, db_1.executeQuery)(`SELECT TaskID, CompanyID FROM dbo.Tasks WHERE TaskID = @taskId AND IsDeleted = 0`, { taskId });
        ...
        const insertResult = await (0, db_1.executeQuery)(`INSERT INTO dbo.TaskComments (TaskID, UserID, CommentText, MentionedUserIDs, CreatedAt, UpdatedAt)
       ...
       VALUES (@taskId, @userId, @commentText, @mentionedUserIds, SYSUTCDATETIME(), SYSUTCDATETIME())`, {
        ...
        // Activity log
        await (0, db_1.executeQuery)(`INSERT INTO dbo.TaskActivityLog (TaskID, UserID, ActionType, Description)
       VALUES (@taskId, @userId, N'CommentAdded', N'Added a conversation comment.')`, { taskId, userId });
...
async function uploadTaskAttachment(req, res, next) {
    try {
        ...
        const insertResult = await (0, db_1.executeQuery)(`INSERT INTO dbo.TaskAttachments (
...
async function approveTask(req, res, next) {
    try {
        const taskId = parseInt(req.params.id, 10);
        const userId = req.user.userId;
        const taskResult = await (0, db_1.executeQuery)(`SELECT TaskID, CompanyID, TaskNumber, TaskTitle, Status FROM dbo.Tasks WHERE TaskID = @taskId AND IsDeleted = 0`, { taskId });
        if (taskResult.recordset.length === 0) {
            res.status(404).json({ success: false, message: 'Task not found.' });
            return;
        }
        ...
        UpdatedAt = SYSUTCDATETIME()
       WHERE TaskID = @taskId`, { taskId, userId });
        await (0, db_1.executeQuery)(`INSERT INTO dbo.TaskActivityLog (TaskID, UserID, ActionType, Description)
       VALUES (@taskId, @userId, N'Approved', N'Task review completed and approved. Status marked as Completed.')`, { taskId, userId });
        // Notify assignees
        const assignees = await (0, db_1.executeQuery)(`SELECT u.UserID 
       FROM dbo.TaskAssignees ta
       JOIN dbo.Users u ON ta.EmployeeID = u.EmployeeID
       WHERE ta.TaskID = @taskId AND u.Status = 'Active'`, { taskId });
...
async function rejectTask(req, res, next) {
    try {
        ...
        UpdatedAt = SYSUTCDATETIME()
       WHERE TaskID = @taskId`, { taskId, reason: reason.trim() });
        await (0, db_1.executeQuery)(`INSERT INTO dbo.TaskActivityLog (TaskID, UserID, ActionType, Description)
...
async function bulkTaskAction(req, res, next) {
    try {
        ...
        const validIds = taskIds.map((id) => parseInt(id, 10)).filter((id) => !isNaN(id));
        const isSuperAdmin = req.tenant?.isSuperAdmin || req.user?.roleName === 'Super Admin' || req.user?.isPlatformAdmin;
        const allowedCompanyIds = req.tenant?.allowedCompanyIds || [req.user.companyId];
        const compFilter = isSuperAdmin ? '' : ` AND CompanyID IN (${allowedCompanyIds.join(', ')})`;
        if (action === 'status') {
            for (const id of validIds) {
                await (0, db_1.executeQuery)(`UPDATE dbo.Tasks SET Status = @value, UpdatedAt = SYSUTCDATETIME() 
           WHERE TaskID = @id ${compFilter}`, { value, id });
                await (0, db_1.executeQuery)(`INSERT INTO dbo.TaskActivityLog (TaskID, UserID, ActionType, Description)
           VALUES (@id, @userId, N'BulkStatusChange', @desc)`, { id, userId, desc: `Bulk status update to ${value}` });
            }
        }
        else if (action === 'priority') {
            for (const id of validIds) {
                await (0, db_1.executeQuery)(`UPDATE dbo.Tasks SET Priority = @value, UpdatedAt = SYSUTCDATETIME() 
           WHERE TaskID = @id ${compFilter}`, { value, id });
            }
        }
        else if (action === 'delete') {
            for (const id of validIds) {
                await (0, db_1.executeQuery)(`UPDATE dbo.Tasks SET IsDeleted = 1, UpdatedAt = SYSUTCDATETIME() 
           WHERE TaskID = @id ${compFilter}`, { id });
            }
        }
        else if (action === 'assign') {
            const empId = parseInt(value, 10);
            for (const id of validIds) {
                await (0, db_1.executeQuery)(`IF NOT EXISTS (SELECT 1 FROM dbo.TaskAssignees WHERE TaskID = @id AND EmployeeID = @empId)
           INSERT INTO dbo.TaskAssignees (TaskID, EmployeeID) VALUES (@id, @empId)`, { id, empId });
            }
        }
        await (0, audit_service_1.logAudit)({
            userId,
...
async function deleteTask(req, res, next) {
    try {
        const taskId = parseInt(req.params.id, 10);
        const userId = req.user.userId;
        const existingTask = await (0, db_1.executeQuery)(`SELECT TaskID, CompanyID FROM dbo.Tasks WHERE TaskID = @taskId AND IsDeleted = 0`, { taskId });
        if (existingTask.recordset.length === 0) {
            res.status(404).json({ success: false, message: 'Task not found.' });
            return;
        }
...
async function getTemplates(req, res, next) {
    try {
        ...
        const templates = await (0, db_1.executeQuery)(`SELECT t.*, d.DepartmentName, c.CategoryName
       FROM dbo.TaskTemplates t
       LEFT JOIN dbo.Departments d ON t.DepartmentID = d.DepartmentID
       LEFT JOIN dbo.TaskCategories c ON t.CategoryID = c.CategoryID
       WHERE t.CompanyID = @companyId
       ORDER BY t.TemplateName ASC`, { companyId });
...
async function createTemplate(req, res, next) {
    try {
        ...
        const insertResult = await (0, db_1.executeQuery)(`INSERT INTO dbo.TaskTemplates (
