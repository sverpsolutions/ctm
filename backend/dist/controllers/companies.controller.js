...
exports.getCompanyHierarchyTree = getCompanyHierarchyTree;
exports.getAccessibleCompanies = getAccessibleCompanies;
 ...
 */
async function getCompanyHierarchyTree(req, res, next) {
    try {
        const isSuperAdmin = req.tenant?.isSuperAdmin;
        const allowedIds = isSuperAdmin ? undefined : req.tenant?.allowedCompanyIds;
        const tree = await tenant_service_1.TenantService.getCompanyTree(allowedIds);
        res.json({
            success: true,
            data: tree,
        });
...
async function getAccessibleCompanies(req, res, next) {
    try {
        ...
        const accessible = await tenant_service_1.TenantService.getUserAccessibleCompanies(userId, roleName);
        res.json({
            success: true,
            data: accessible,
...
async function getCompanies(req, res, next) {
    try {
        ...
        if (parentCompanyId) {
            ...
            params.parentCompanyId = parseInt(parentCompanyId, 10);
        }
        const countResult = await (0, db_1.executeQuery)(`SELECT COUNT(*) AS TotalCount FROM dbo.Companies c WHERE ${whereSql}`, params);
        ...
        const listResult = await (0, db_1.executeQuery)(`SELECT 
...
async function getCompanyById(req, res, next) {
    try {
        ...
        const companyResult = await (0, db_1.executeQuery)(`SELECT c.*, p.CompanyName AS ParentCompanyName
       FROM dbo.Companies c
       LEFT JOIN dbo.Companies p ON c.ParentCompanyID = p.CompanyID
       WHERE c.CompanyID = @companyId AND c.IsDeleted = 0`, { companyId });
        ...
        company.EnabledModules = company.EnabledModules
            ...
            : ['tasks', 'dates', 'calendar', 'reports', 'audit'];
        // Child companies
        const childrenResult = await (0, db_1.executeQuery)(`SELECT CompanyID, CompanyCode, CompanyName, CompanyType, Status,
              (SELECT COUNT(DISTINCT u.UserID) 
               FROM dbo.Users u 
               LEFT JOIN dbo.UserCompany uc ON u.UserID = uc.UserID AND uc.IsActive = 1
               WHERE (u.CompanyID = c.CompanyID OR uc.CompanyID = c.CompanyID) AND u.IsDeleted = 0) AS UserCount
       FROM dbo.Companies c
       WHERE c.ParentCompanyID = @companyId AND c.IsDeleted = 0`, { companyId });
        // Mapped Users
        const usersResult = await (0, db_1.executeQuery)(`SELECT uc.UserCompanyID, uc.UserID, uc.RoleID, uc.AccessScope, uc.IsPrimary, uc.IsActive,
       ...
       WHERE uc.CompanyID = @companyId AND u.IsDeleted = 0`, { companyId });
        res.json({
            success: true,
            data: {
                ...company,
...
async function createCompany(req, res, next) {
    try {
        const userId = req.user.userId;
        const { companyName, companyCode, legalName, companyType = 'Subsidiary', parentCompanyId, address, city, state, country = 'India', pinCode, phone, email, website, gstin, pan, logo, enabledModules = ['tasks', 'dates', 'calendar', 'reports', 'audit'], subscriptionTier = 'Enterprise', maxUsers = 100, } = req.body;
        if (!companyName || !companyCode) {
            res.status(400).json({ success: false, message: 'Company name and company code are required.' });
            return;
        }
        // Check duplicate code
        const existing = await (0, db_1.executeQuery)(`SELECT CompanyID FROM dbo.Companies WHERE CompanyCode = @companyCode AND IsDeleted = 0`, { companyCode: companyCode.trim().toUpperCase() });
        ...
        const insertResult = await (0, db_1.executeQuery)(`INSERT INTO dbo.Companies (
      ...
      VALUES (
       ...
       VALUES (@userId, @newCompanyId, 1, N'Global', 0, 1)`, { userId, newCompanyId });
...
async function updateCompany(req, res, next) {
    try {
        ...
        const { companyName, legalName, companyType, parentCompanyId, address, city, state, country, pinCode, phone, email, website, gstin, pan, logo, enabledModules, subscriptionTier, maxUsers, status, } = req.body;
 ...
 */
async function assignUserToCompany(req, res, next) {
    try {
        const companyId = parseInt(req.params.id, 10);
        ...
        if (isPrimary) {
            ...
            await (0, db_1.executeQuery)(`UPDATE dbo.UserCompany SET IsPrimary = 0 WHERE UserID = @userId`, { userId });
            // Update primary company in Users and Employees tables
            await (0, db_1.executeQuery)(`UPDATE dbo.Users SET CompanyID = @companyId WHERE UserID = @userId`, { companyId, userId });
            await (0, db_1.executeQuery)(`UPDATE dbo.Employees SET CompanyID = @companyId 
         WHERE EmployeeID = (SELECT EmployeeID FROM dbo.Users WHERE UserID = @userId)`, { companyId, userId });
        }
        else {
            // If user currently has no company assigned in Users table, set this as CompanyID
            await (0, db_1.executeQuery)(`UPDATE dbo.Users SET CompanyID = @companyId WHERE UserID = @userId AND (CompanyID IS NULL OR CompanyID = 0)`, { companyId, userId });
        }
        // Upsert UserCompany mapping
        const existing = await (0, db_1.executeQuery)(`SELECT UserCompanyID FROM dbo.UserCompany WHERE UserID = @userId AND CompanyID = @companyId`, { userId, companyId });
        if (existing.recordset.length > 0) {
          ...
          IsActive = 1,
          UpdatedAt = SYSUTCDATETIME()
         WHERE UserID = @userId AND CompanyID = @companyId`, { userId, companyId, roleId, accessScope, isPrimary: isPrimary ? 1 : 0 });
        }
        else {
            await (0, db_1.executeQuery)(`INSERT INTO dbo.UserCompany (UserID, CompanyID, RoleID, AccessScope, IsPrimary, IsActive)
         VALUES (@userId, @companyId, @roleId, @accessScope, @isPrimary, 1)`, { userId, companyId, roleId, accessScope, isPrimary: isPrimary ? 1 : 0 });
        }
        res.json({
            success: true,
 ...
 */
async function removeUserFromCompany(req, res, next) {
    try {
        const companyId = parseInt(req.params.id, 10);
        const userId = parseInt(req.params.userId, 10);
        await (0, db_1.executeQuery)(`DELETE FROM dbo.UserCompany WHERE UserID = @userId AND CompanyID = @companyId`, { userId, companyId });
        res.json({
            success: true,
